...
import { ChevronDown, ChevronRight, ChevronUp, ChevronLeft, Calendar as CalIcon, Clock, X } from "lucide-react"
import {
  ACADEMIC_YEARS,
  CLASSES,
  DEMAND_PRESETS,
...
function deriveAssignmentType(sections?: AssessmentSection[]): Assignment["type"] {
  if (!sections || sections.length === 0) return "mcq"
  const types = sections.map((s) => s.type)
  if (types.some((t) => t === "mcq" || t === "multi-select" || t === "true-false" || t === "matching")) {
    return "mcq"
  }
  if (types.some((t) => t === "coding")) {
    return "coding"
  }
  return "written"
}

function buildAnswerKey(sections?: AssessmentSection[]): Record<string, string> {
  if (!sections) return {}
  ...
  sections.forEach((sec, sIdx) => {
    sec.questions.forEach((q, qIdx) => {
      ...
      if (q.options && q.options.length > 0) {
        // Find the first option marked correct
        const correctOpt = q.options.find((opt: any) => opt?.correct === true)
        if (correctOpt) {
          const textVal = typeof correctOpt === "string" ? correctOpt : (correctOpt?.text || "")
          key[qId] = textVal
        }
      } else if (q.correctAnswer) {
        key[qId] = q.correctAnswer
      }
    })
  })
  return key
...
function formatAssessmentDescription(sections?: AssessmentSection[]): string {
  if (!sections || sections.length === 0) return ""
  const answerKey = buildAnswerKey(sections)
  const parts: string[] = []
  // Embed answer key as JSON at top so backend can read it for auto-grading
  if (Object.keys(answerKey).length > 0) {
    parts.push(`__ANSWER_KEY__:${JSON.stringify(answerKey)}`)
  }
  sections.forEach((sec, sIdx) => {
    sec.questions.forEach((q, qIdx) => {
      let qText = `${sIdx + 1}.${qIdx + 1}) ${q.text}`
      if (q.options && q.options.length > 0) {
        const optsText = q.options.map((opt: any, oIdx) => {
          const textVal = typeof opt === "string" ? opt : (opt?.text || opt?.label || "")
          const letter = String.fromCharCode(65 + oIdx)
          return `${letter}) ${textVal}`
        }).join("\n")
        qText += "\n" + optsText
      }
      parts.push(qText)
...
export default function AssessmentBuilder() {
  ...
  // Assign-from-bank modal & Date/Time picker state
  ...
  const [dueHour12, setDueHour12] = useState<number>(11)
  ...
  /* ---- Generate / Save ---- */
  const ready = objectiveText.trim().length > 0 && sections.length > 0
  ...
  const confirmAssign = () => {
    ...
    const assignmentType = deriveAssignmentType(entry.sections)
    const compiledDescription = formatAssessmentDescription(entry.sections)
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={stepCard}>
              ...
              <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px solid #EFEBE2" }}>
                ...
                />
                {!objectiveText.trim() && (
                  <div style={{ fontSize: 12, color: "#D48A0C", marginTop: 4 }}>
                    💡 Enter a learning objective or title above to enable saving this assessment.
                  </div>
                )}
              </div>
            </div>
      ...
      {assignModal && (
        <div onClick={() => setAssignModal(null)} style={modalOverlay}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#F5F1E6", border: "1px solid #DDD8CF", borderRadius: 16, boxShadow: "0 24px 60px rgba(26,46,53,.22)", width: "100%", maxWidth: 440, padding: 22 }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: "#7A9298", marginBottom: 6 }}>Due Date &amp; Time</div>
            <div style={{ position: "relative", marginBottom: 14 }}>
              <button
                type="button"
                onClick={() => setShowCalendar((v) => !v)}
                style={{ display: "flex", height: 40, width: "100%", alignItems: "center", justifyContent: "space-between", borderRadius: 8, border: "1px solid #DDD8CF", backgroundColor: "#fff", padding: "0 12px", textAlign: "left", fontSize: 13.5, cursor: "pointer" }}
              >
                <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                  <CalIcon size={16} color="#7A9298" /> {dueLabel}
                </span>
                <Clock size={16} color="#7A9298" />
              </button>

              {showCalendar && (
                <div style={{ position: "absolute", right: 0, top: "100%", zIndex: 60, marginTop: 8, width: 300, borderRadius: 16, border: "1px solid #DDD8CF", backgroundColor: "#fff", padding: 16, boxShadow: "0 12px 30px rgba(0,0,0,0.15)" }}>
                  {calStep === "date" ? (
                    <>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                        <button
                          type="button"
                          onClick={prevMonth}
                          style={{ display: "flex", height: 28, width: 28, alignItems: "center", justifyContent: "center", borderRadius: "50%", border: "none", background: "transparent", cursor: "pointer" }}
                        >
                          <ChevronLeft size={16} />
                        </button>
                      ...
                      {/* Day-of-week labels */}
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4, textAlign: "center", fontSize: 11, color: "#7A9298", marginBottom: 4 }}>
                        {["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"].map((d) => (
                          <div key={d} style={{ height: 28, display: "grid", placeItems: "center" }}>{d}</div>
                        ))}
                      </div>
                      {/* Day grid */}
                          ...
                          return (
                            <button
                              ...
                              style={{
                                ...
                                background: isSelected ? "#16332B" : "transparent",
                    ...
                    <>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                        ...
                        <div style={{ fontSize: 13, fontWeight: 600 }}>Select Time</div>
                        <div style={{ width: 28 }} />
                      </div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16, marginTop: 16 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                          <button
                            ...
                            style={{ height: 24, width: 40, display: "grid", placeItems: "center", border: "none", background: "transparent", cursor: "pointer" }}
                          >
                            <ChevronLeft size={14} style={{ transform: "rotate(90deg)" }} />
                          </button>
                          <div style={{ display: "flex", height: 36, width: 48, alignItems: "center", justifyContent: "center", borderRadius: 8, border: "1px solid #DDD8CF", backgroundColor: "#F5F1E6", fontSize: 16, fontWeight: 700 }}>
                        ...
                        {/* AM/PM toggle */}
                        <div style={{ display: "flex", flexDirection: "column", gap: 4, border: "1px solid #DDD8CF", borderRadius: 8, padding: 4, backgroundColor: "#F5F1E6" }}>
                          {(["AM", "PM"] as const).map((period) => (
                            <button
                              ...
                              style={{
                                ...
                                border: "none",
                                cursor: "pointer",
                                background: duePeriod === period ? "#16332B" : "transparent",
                                color: duePeriod === period ? "#fff" : "#4B5563",
                              }}
                            >
                              {period}
                            </button>
                      ...
                      <button
                        ...
                        style={{ marginTop: 24, width: "100%", borderRadius: 8, padding: "8px 0", fontSize: 13, fontWeight: 600, color: "#fff", background: "#16332B", border: "none", cursor: "pointer" }}
                      >
                        Done
                      </button>
                    </>
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#F5F1E6", border: "1px solid #DDD8CF", borderRadius: 16, boxShadow: "0 24px 60px rgba(26,46,53,.22)", width: "100%", maxWidth: 480, maxHeight: "88vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            <div style={{ padding: 20, overflowY: "auto" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 12px", background: "#FFFFFF", borderRadius: 8 }}>
                ...
                <span style={{ width: 10, height: 10, borderRadius: "50%", background: objectiveText.trim().length > 0 ? "#2E9E6B" : "#D48A0C" }} />
              </div>
            </div>
