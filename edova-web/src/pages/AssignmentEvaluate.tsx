...
export default function AssignmentEvaluate() {
  ...
  const evaluatedQuestions = useMemo(() => {
    ...
    // Structured MCQ submissions (via McqQuiz -> submitQuiz) store answers as
    // {question_id, selected}[] instead of textResponse — merge those in too.
    if (selected.answers && selected.answers.length > 0) {
      for (const a of selected.answers) {
        if (a?.question_id) answersMap[a.question_id] = a.selected
      }
    }
